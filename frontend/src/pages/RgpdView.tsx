...
export default function RgpdView() {
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      {/* SECTION — LA PREUVE PAR LE MONITORING */}
      <div className="mb-16 sm:mb-20">
        <span className="inline-block text-[11px] font-bold uppercase tracking-widest text-violet-600 mb-3">
          Après la livraison
        </span>
        <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-2 leading-tight">
          La preuve par le monitoring — ce qui tourne 24/7 pour vous
        </h2>
        <p className="text-sm text-gray-600 mb-6 sm:mb-8 max-w-3xl leading-relaxed">
          Un outil sur-mesure, ça peut évoluer — et ce qui évolue peut casser. C'est pour ça qu'on ne se contente pas
          de vous livrer Synthèse. <span className="font-semibold text-gray-900">4 outils de surveillance indépendants</span> tournent
          en continu sur votre instance, avec alertes instantanées dès qu'un comportement anormal apparaît.
          Objectif : qu'on sache qu'il y a un problème <span className="font-semibold text-gray-900">avant vous</span>.
        </p>
          ...
          {[
            {
              ...
              desc: "Des sentinelles automatisées analysent votre instance 24h/24, 7j/7 — performance, erreurs, disponibilité, dépendances.",
              color: "from-violet-500 to-violet-600",
            },
            {
              num: "2",
              icon: Zap,
              title: "On détecte avant vous",
              desc: "Dès qu'une anomalie survient (même minime), on reçoit une alerte. On intervient immédiatement, souvent avant même que vous ne l'aperceviez.",
              color: "from-orange-500 to-orange-600",
            },
            {
              num: "3",
              icon: LineChart,
              title: "On vous rend des comptes",
              desc: "Rapport mensuel automatique : uptime réel, incidents résolus, correctifs appliqués. Tout est tracé, rien n'est caché.",
              color: "from-emerald-500 to-emerald-600",
            },
          ].map((step) => {
            const Icon = step.icon;
        ...
        {/* Les 4 outils de surveillance */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
          {[
            {
              name: "Sentry",
              role: "Détection d'erreurs en production",
              color: "#c96442",
              icon: Bug,
              desc: "Capture chaque erreur qui survient dans votre Synthèse — même silencieuse. Alerte instantanée avec la ligne de code responsable, prête à corriger.",
              metric: "< 5 sec",
              metricLabel: "Délai d'alerte",
            },
            {
              name: "UptimeRobot",
              role: "Surveillance de disponibilité 24/7",
              color: "#2563eb",
              icon: Activity,
              desc: "Vérifie toutes les minutes que votre Synthèse répond correctement. En cas d'indisponibilité, alerte SMS et email immédiate.",
              metric: "99.9%",
              metricLabel: "SLA d'uptime",
            },
            {
              name: "Dependabot",
              role: "Correctifs de sécurité automatiques",
              color: "#7c3aed",
              icon: GitBranch,
              desc: "Scanne chaque jour les librairies utilisées. Dès qu'une faille est publiée, on applique le correctif sous 24h — avant qu'elle ne soit exploitée.",
              metric: "< 24h",
              metricLabel: "Délai de patch",
            },
            {
              name: "Supabase PITR",
              role: "Restauration à la minute près",
              color: "#059669",
              icon: History,
              desc: "Point-in-time recovery : en cas de problème (erreur humaine, bug, corruption), on peut restaurer votre instance à n'importe quelle minute des 7 derniers jours.",
              metric: "1 min",
              metricLabel: "Granularité",
            },
          ].map((tool) => {
            ...
            return (
              <div
                ...
                className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-md hover:border-gray-300 transition-all"
              >
                <div className="h-1.5 w-full" style={{ backgroundColor: tool.color }} />
                  ...
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] sm:text-xs text-gray-500 truncate">{tool.role}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <div
                        className="inline-flex items-center justify-center min-w-[60px] h-9 px-2.5 rounded-lg font-bold text-xs sm:text-sm"
                        style={{
                          color: tool.color,
                      ...
                      <p className="text-[10px] text-gray-400 mt-1">{tool.metricLabel}</p>
                    </div>
                  </div>
        ...
        {/* SLA contractuel + status page */}
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
          {/* SLA */}
          <div className="rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 p-5 sm:p-6">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center shrink-0">
                <FileText className="h-5 w-5 text-emerald-700" />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Engagement contractuel</p>
                <p className="text-base font-semibold text-gray-900">SLA de 99.9% uptime</p>
              </div>
            </div>
            <p className="text-[13px] sm:text-xs text-gray-600 leading-relaxed">
              99.9% d'uptime garanti, écrit noir sur blanc dans votre contrat. Si on descend en dessous, vous êtes dédommagé automatiquement.
              Ce n'est pas un vœu pieux — c'est un engagement mesuré et vérifié chaque mois.
            </p>
          </div>

          {/* Status page */}
          <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 p-5 sm:p-6">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center shrink-0">
                <Activity className="h-5 w-5 text-blue-700" />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-blue-700">Transparence totale</p>
                <p className="text-base font-semibold text-gray-900">Status page publique</p>
              </div>
            </div>
            <p className="text-[13px] sm:text-xs text-gray-600 leading-relaxed">
              Une page publique accessible à tout moment (comme celle de Stripe ou OpenAI). Vous y voyez en temps réel l'état de votre Synthèse,
              l'uptime des 90 derniers jours, et l'historique des incidents. Abonnement par email aux alertes possible.
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 rounded-2xl bg-gradient-to-br from-violet-50 to-blue-50 border border-violet-100 px-5 py-5 sm:px-7 sm:py-6">
          <div className="flex items-start gap-3 sm:gap-4">
            <Award className="h-5 w-5 sm:h-6 sm:w-6 text-violet-600 shrink-0 mt-0.5" />
            <p className="text-[13px] sm:text-sm text-gray-700 leading-relaxed">
              La plupart des logiciels SaaS{" "}
              <span className="font-semibold text-gray-900">cassent en silence</span>{" "}
              — vous vous en apercevez quand un client râle, quand un rapport ne sort pas, ou quand une facture ne part plus.
              Chez Synthèse, c'est l'inverse : si quelque chose bouge, on est alerté avant vous, et très souvent,
              c'est corrigé avant que vous n'ayez eu le temps de le remarquer.
            </p>
          </div>
        </div>
      </div>
